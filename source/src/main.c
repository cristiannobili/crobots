...
int UPDATE_CYCLES = 1; /* number of cycles before screen update */
...
char *version = "CROBOTS - version 1.2 (2022) by Cristian Nobili \n";
...
char *argv[];
{
  ...
  if (comp_only)
    ...
    if (debug_only)
      tracef(files[0]); /* trace only first source */
    else
    {
      if (num_robots < 2)
...
int n;
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    else
    {
      ...
      fwrite(&(robots[i].code), sizeof(long), 1, f_out); /*questo � un valore di offset*/
...
int n;
{
...
#endif
  r_debug = 0;
  ndebug = 0; /* turns off full compile info */
  ...
  for (m_count = 1; m_count <= m; m_count++)
  {
    ...
    /*
    In this section there is the final log. 
    */
    for (i = 0; i < n; i++)
    {
      if (robots[i].status == ACTIVE)
